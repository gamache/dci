## This class was written by Peter Cooper.  It was taken from:
...
class RDParser
  ...
  def parse(rule, content, options = {})
    ...
    @content = Scanner.new(content, :slurp_whitespace => !options[:manual_whitespace])
    
    # Kick off the show by parsing from the rule specified
    parse_section(rule.to_sym).flatten
  end
